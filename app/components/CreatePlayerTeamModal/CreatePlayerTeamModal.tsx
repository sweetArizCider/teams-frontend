...
import type { CreatePlayerTeamRequest, Player, Team } from '../../interfaces/server';
import { useCreatePlayerTeam } from '../../hooks/playersTeam/useCreatePlayerTeam';

interface CreatePlayerTeamModalProps {
  isOpen: boolean;
  onClose: () => void;
  onPlayerTeamCreated: (_newPlayerTeam: any) => Promise<void>;
...
export const CreatePlayerTeamModal: React.FC<CreatePlayerTeamModalProps> = ({
  ...
  const { createPlayerTeam, loading, error, clearError } = useCreatePlayerTeam();
  ...
  const handleSelectAllPlayers = (checked: boolean): void => {
    if (checked) {
      const allPlayerIds = new Set(paginatedPlayers.map(player => player._id!));
      setSelectedPlayers(prev => new Set([...prev, ...allPlayerIds]));
    } else {
      const currentPagePlayerIds = new Set(paginatedPlayers.map(player => player._id!));
    ...
    try {
      ...
      for (const playerId of selectedPlayers) {
        const playerTeamData: CreatePlayerTeamRequest = {
          ...
          position: 'TBD', // Default position to be updated later
          jersey_number: 0, // Default jersey number to be updated later
          start_date: new Date().toISOString().split('T')[0], // Today's date
          is_active: true
        };

        const result = await createPlayerTeam(playerTeamData);
        if (result) {
      ...
      if (createdPlayerTeams.length > 0) {
        await onPlayerTeamCreated(createdPlayerTeams[0]); // Pass the first created relationship
        handleClose();
      }
    } catch (err) {
            ...
            <div>
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Select players to add to the team. Position, jersey number, and other details can be edited later in the Player Teams tab.
                </p>
              </div>
                      ...
                      >
                        <TableCell className="p-4">
                          <Checkbox
                            ...
                            onChange={(e) => handlePlayerSelection(player._id!, e.target.checked)}
                          />
                        </TableCell>
